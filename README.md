...
- 一个服务对应一个目录和一个支持注释的 JSON5 `.served.json`。
- 服务的工作目录固定为配置目录。
- 配置中的 `env` 覆盖 manager 启动环境；旧版 `.env.served` 只作为兼容性回退。
  项目 `.env` 不会被 served 读取。
- 默认使用 PTY。设置 `tty: false` 后改用 stdout/stderr 管道。
- TTY attach 默认把服务 PTY 的 rows/cols 同步为当前终端尺寸。可用
  `syncRowsCols: false` 关闭。
- 支持 `never`、`on-failure`、`always` 三种重启策略，并使用指数退避。
- 服务在 60 秒内失败至少 3 次后，attach 会提示崩溃循环，并可打开持久化日志。
- PTY 服务支持可写 attach。pipe 服务支持只读 attach。所有 attach 都使用终端第二屏。
- `served attach [name]` 可以绕过 TUI，直接进入 attach 会话。
- 全局 TUI 显示服务状态，并支持 restart、disable、attach、历史和随机 `tips:`。
- 日志可以按服务持久化，也可以只保存在 runner 内存中。
...
`served.service`、`install.sh`、`uninstall.sh` 和本 README。每个产物都有自己的
