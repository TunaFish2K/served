...
`s` start、`x` stop、`r` restart、`e` edit、`h` history、`d` disable。禁用需要确认，默认取消。
...
文件在外部被修改时会拒绝覆盖。保存不会启用服务。有头表单实际保存修改后，若对应服务正在运行，会询问 Not now／Restart，默认 Not now。面板 Edit 与直接 served edit 共用此流程；外部编辑器和无头版不增加询问。
