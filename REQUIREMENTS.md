...
- 支持 40×10、80×24、120×40，按显示宽度截断名称和路径，操作菜单的 `?` 帮助中可查看完整信息。
...
- 提供英文 `served(1)` 命令手册、`served(5)` 配置手册和通用 `served` skill，中英文 README
  提供入口。skill 使用的详细参考从 mdoc 生成，随仓库保存且可离线使用。
- 完整安装包包含手册与 skill；Release 提供带 SHA-256 的独立 skill 归档。
- 安装、修复、升级和回滚包含共享文档。仅文档变化不重启 manager 或服务；多用户卸载
  遵循共享二进制的保留规则。用户选择其 AI 工具的技能导入位置。
