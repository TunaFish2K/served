...
- CLI `served history [name]` 未提供 `--run <id>` 时选择 `latest`。没有输出模式时，它用
  共享编辑器解析器打开持久化 raw 文件；`--path` 只打印持久化路径。`--stdout` 通过分页
  IPC 流式输出清理后的内容，`--json` 输出内容及 `service`、`id`、`current`、`persisted`、
  `raw_bytes`、`total_lines` 元数据。这两种模式都支持持久化和内存记录，不创建临时文件。
  四种输出模式互斥。
...
历史显示和 CLI 导出适合终端、脚本及 AI 工具读取，但不会复现 PTY 的终端状态、颜色或
光标行为。CLI 编辑器流程仍使用持久化 raw 文件，因此用户也可以检查准确字节和终端控制
数据。不持久化服务在 runner 生命周期内仍有可自动化读取的临时历史。
